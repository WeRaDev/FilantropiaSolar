...
MAX_PERCENT = 100.0  # Upper bound for percentage-based weather metrics
...
def validate_weather_data(
    ...
    if np.any((cloud_cover < 0) | (cloud_cover > MAX_PERCENT)):
        warnings.append("Cloud cover outside 0-100% range - clipping")
        cloud_cover = np.clip(cloud_cover, 0, MAX_PERCENT)

    # Check humidity bounds
    if np.any((humidity < 0) | (humidity > MAX_PERCENT)):
        warnings.append("Humidity outside 0-100% range - clipping")
        humidity = np.clip(humidity, 0, MAX_PERCENT)
