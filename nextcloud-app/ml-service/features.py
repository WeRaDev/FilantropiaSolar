...
# Time-of-day boundaries (hours) for time-based indicators
PEAK_SUN_START_HOUR = 10
PEAK_SUN_END_HOUR = 14
SOLAR_NOON_HOUR = 12
HOURS_PER_DAY = 24
...
def enhance_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    enhanced["temp_rolling_avg"] = (
        enhanced["temperature_2m"].rolling(ROLLING_WINDOW_HOURS, min_periods=1).mean()
    )
    ...
    enhanced["radiation_cloud_interaction"] = enhanced["shortwave_radiation"] * (
        1 - enhanced["cloud_cover"] / 100
    )
    ...
    enhanced["radiation_humidity_interaction"] = enhanced["shortwave_radiation"] * (
        1 - enhanced["relative_humidity_2m"] / 100
    )
    ...
    enhanced["radiation_sqrt"] = np.sqrt(enhanced["shortwave_radiation"].clip(lower=0))

    # Time-based indicators
    hour = enhanced.index.hour
    enhanced["peak_sun_indicator"] = (
        (hour >= PEAK_SUN_START_HOUR) & (hour <= PEAK_SUN_END_HOUR)
    ).astype(float)
    enhanced["morning_ramp"] = np.where(
        hour < SOLAR_NOON_HOUR, hour / SOLAR_NOON_HOUR, 0
    )
    enhanced["evening_ramp"] = np.where(
        hour >= SOLAR_NOON_HOUR,
        (HOURS_PER_DAY - hour) / SOLAR_NOON_HOUR,
        0,
    )
