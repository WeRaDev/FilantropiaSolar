...
def calculate_correlations(dfs, location, weather_location):
    ...
    merged_df = pd.merge_asof(
        production_df.sort_values("Date"),
    ...
    plt.title(
        f"Correlation Matrix for Weather Features vs. Produced Energy in {location}",
        fontsize=16,
    )
...
def plot_3d_energy_production(df, plant_name):
    ...
    average_energy = (
        df.groupby(["Hour", "Month"])["Produced Energy (kWh)"]
        .mean()
        .reset_index()
        .astype(float)
    )
...
def plot_daily_energy_trends_combined(dfs):
    ...
    for plant_name, df in dfs.items():
        ...
        daily_energy = (
            df.groupby("Day_of_Year")["Produced Energy (kWh)"].sum().reset_index()
        )

        sns.lineplot(
            x="Day_of_Year",
            y="Produced Energy (kWh)",
            data=daily_energy,
            label=plant_name,
        )
