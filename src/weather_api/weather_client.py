...
class WeatherClient:
    ...
    def get_weather_for_date_range(self, target_date, days_before=7, days_after=7):
        ...
        if end_date <= today.strftime("%Y-%m-%d"):
            ...
            days_ahead = (datetime.strptime(end_date, "%Y-%m-%d").date() - today).days + 1
    ...
    def _get_default_forecast_df(self, days_ahead):
        ...
        return pd.DataFrame({
            ...
            "shortwave_radiation (W/m²)": [200.0 if 8 <= date.hour <= 18 else 0.0 for date in dates]
        })
    ...
    def get_current_weather(self):
        ...
        try:
            ...
            return {
                ...
                "wind_speed_10m": data.get("wind", {}).get("speed", 5) * 3.6,  # Convert m/s to km/h
