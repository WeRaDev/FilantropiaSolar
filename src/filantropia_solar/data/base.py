...
class DataLoader(Protocol[T]):
    ...
    def load(self, source: Union[str, Path], **kwargs) -> T:
...
class DataExporter(Protocol[T]):
    ...
    def export(self, data: T, destination: Union[str, Path], format: str = "csv", **kwargs) -> bool:
...
class BaseDataProcessor(abc.ABC, Generic[T]):
    ...
    def __init__(self, name: str):
        ...
        self._last_quality_report: Optional[DataQualityReport] = None
    
    @abc.abstractmethod
    def _load_data(self, source: Union[str, Path], **kwargs) -> T:
    ...
    def _preprocess_source(self, source: Union[str, Path]) -> Union[str, Path]:
    ...
    def process(
        ...
        **kwargs
        ...
        try:
            ...
            duration = (datetime.utcnow() - start_time).total_seconds()
            self.logger.error(f"Data processing failed after {duration:.2f}s: {str(e)}")
    ...
    def get_last_quality_report(self) -> Optional[DataQualityReport]:
...
class PandasDataProcessor(BaseDataProcessor[pd.DataFrame]):
    ...
    def _load_data(self, source: Union[str, Path], **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            if isinstance(e, DataLoadingError):
                raise
            raise DataLoadingError(
                f"Failed to load data from {source}: {str(e)}",
                file_path=str(source),
                data_source="file",
                cause=e
            ) from e
