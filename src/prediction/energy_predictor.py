...
class EnergyPredictor:
    ...
    def prepare_features(self, df):
        ...
        if 'Date' in features_df.columns:
            ...
            features_df['SinDayOfYear'] = np.sin(2 * np.pi * features_df['DayOfYear'] / 365)
            features_df['CosDayOfYear'] = np.cos(2 * np.pi * features_df['DayOfYear'] / 365)
        
        # Interaction features
        if 'shortwave_radiation (W/m²)' in features_df.columns and 'cloud_cover (%)' in features_df.columns:
            features_df['radiation_cloud_interaction'] = features_df['shortwave_radiation (W/m²)'] * (100 - features_df['cloud_cover (%)']) / 100
        
        if 'temperature_2m (°C)' in features_df.columns and 'relative_humidity_2m (%)' in features_df.columns:
            features_df['temp_humidity_interaction'] = features_df['temperature_2m (°C)'] / (features_df['relative_humidity_2m (%)'] + 1)
        ...
        if 'Hour' in features_df.columns and 'DayOfYear' in features_df.columns:
            ...
            features_df['SolarElevation'] = np.maximum(0, 
                np.sin(np.pi * (features_df['Hour'] - 6) / 12) * 
                np.sin(np.pi * (features_df['DayOfYear'] - 80) / 365)
            )
        
        # Select final feature columns
        time_features = ['Hour', 'Month', 'SinHour', 'CosHour', 'SinMonth', 'CosMonth', 
                        'SinDayOfYear', 'CosDayOfYear', 'Season', 'DayOfWeek']
        interaction_features = ['radiation_cloud_interaction', 'temp_humidity_interaction', 'SolarElevation']
    ...
    def train_models(self, training_data):
        ...
        if valid_results:
            self.best_model_name = min(valid_results.keys(), key=lambda k: valid_results[k]['mae'])
    ...
    def predict_energy(self, weather_data, installed_capacity_kwp=10.0):
        ...
        try:
            ...
            results_df['Specific Energy (kWh/kWp)'] = predictions / installed_capacity_kwp
            results_df['Ranking'] = calculate_specific_energy_ranking(results_df['Specific Energy (kWh/kWp)'])
    ...
    def get_daily_summary(self, predictions_df):
        ...
        daily_summary = predictions_df.groupby('Date').agg({
            ...
            'Ranking': 'mean',
            'temperature_2m (°C)': 'mean' if 'temperature_2m (°C)' in predictions_df.columns else lambda x: np.nan,
            'shortwave_radiation (W/m²)': 'mean' if 'shortwave_radiation (W/m²)' in predictions_df.columns else lambda x: np.nan,
            'cloud_cover (%)': 'mean' if 'cloud_cover (%)' in predictions_df.columns else lambda x: np.nan
        }).round(2)
        
        # Flatten column names
        daily_summary.columns = ['_'.join(col).strip('_') if isinstance(col, tuple) else col for col in daily_summary.columns]
