...
class FilantropiaSolarApp:
    ...
    def create_input_window(self, parent):
        ...
        # Date selection
        ttk.Label(input_frame, text="Target Date:").grid(row=1, column=0, sticky="w", pady=2)
    ...
    def create_plot_window(self, parent):
        ...
        # Plot Frame
        plot_frame = ttk.LabelFrame(parent, text="Energy Production Chart (±7 days)", padding=10)
    ...
    def generate_prediction(self):
        ...
        try:
            ...
            if predictor is None or not predictor.is_trained:
                messagebox.showerror("Error", f"No trained model available for {installation}")
                return
    ...
    def update_plot(self):
        ...
        try:
            ...
            if target_date in daily_data['date'].values:
                ...
                ax1.axvline(x=target_date, color='green', linestyle='--', linewidth=2, alpha=0.7)
                ax1.text(target_date, daily_data.iloc[target_idx]['Predicted Energy (kWh)'] * 1.1, 
                        'TARGET', ha='center', va='bottom', fontweight='bold', color='green')
