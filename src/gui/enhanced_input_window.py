...
class EnhancedInputWindow:
    ...
    def __init__(self, parent, data_processor: ComprehensiveDataProcessor, 
    ...
    def _create_widgets(self):
        ...
        # Title
        title_label = ttk.Label(main_frame, text="Solar Energy Prediction", 
                               font=('Arial', 16, 'bold'))
    ...
    def _create_installation_section(self, parent, row):
        ...
        # Section frame
        install_frame = ttk.LabelFrame(parent, text="Installation Selection", padding="10")
        install_frame.grid(row=row, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(0, 10))
        install_frame.columnconfigure(1, weight=1)
        
        # Installation dropdown
        ttk.Label(install_frame, text="Installation:").grid(row=0, column=0, sticky=tk.W, padx=(0, 10))
        
        installation_names = [f"{info.location}_{info.serial_number} ({info.installed_power_kwp} kWp)" 
                             for _, info in self.installations]
        self.installation_combo = ttk.Combobox(install_frame, textvariable=self.selected_installation,
                                              values=installation_names, state="readonly", width=50)
        ...
        self.capacity_label = ttk.Label(details_frame, text="-", font=('Arial', 10, 'bold'))
        ...
        self.location_label = ttk.Label(details_frame, text="-", font=('Arial', 10, 'bold'))
        ...
        self.coordinates_label.grid(row=1, column=1, columnspan=3, sticky=tk.W, padx=(5, 0))
    ...
    def _create_date_section(self, parent, row):
        ...
        ttk.Label(mode_frame, text="Date Mode:").grid(row=0, column=0, sticky=tk.W, padx=(0, 10))
        
        ttk.Radiobutton(mode_frame, text="Historical Data", variable=self.date_mode, 
                       value="historical").grid(row=0, column=1, sticky=tk.W, padx=(0, 20))
        
        ttk.Radiobutton(mode_frame, text="Future/Simulation", variable=self.date_mode, 
                       value="future").grid(row=0, column=2, sticky=tk.W)
        
        # Date selection
        ttk.Label(date_frame, text="Select Date:").grid(row=1, column=0, sticky=tk.W, padx=(0, 10))
        ...
        self.date_info_label.grid(row=2, column=0, columnspan=3, sticky=tk.W, pady=(5, 0))
    ...
    def _setup_callbacks(self):
        ...
        self.installation_combo.bind('<<ComboboxSelected>>', lambda e: self._on_installation_changed())
    ...
    def _on_installation_changed(self):
        ...
        try:
            ...
            if installation_info:
                ...
                self.capacity_label.config(text=f"{installation_info.installed_power_kwp} kWp")
                ...
                logger.info(f"Selected installation: {installation_info.installation_id}")
        ...
        try:
            ...
            if mode == "historical":
                ...
                if self.historical_date_range[0] and self.historical_date_range[1]:
                    ...
                    mid_date = self.historical_date_range[0] + (
                        self.historical_date_range[1] - self.historical_date_range[0]
                    ) / 2
                ...
                self.date_info_label.config(
                    text="Future dates will use weather simulation",
                    foreground="orange"
                )
    ...
    def _on_date_changed(self):
        ...
        try:
            ...
            is_historical = self.data_processor.is_date_in_historical_range(selected_datetime)
    ...
    def _on_predict_clicked(self):
        ...
        try:
            ...
            # Update status
            self.status_label.config(
                text=f"Prediction generated for {selected_date}",
                foreground="green"
            )
    ...
    def _on_clear_clicked(self):
        ...
        self.status_label.config(
            text="Selection cleared. Choose installation and date.",
            foreground="blue"
        )
