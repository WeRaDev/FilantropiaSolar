...
class EnhancedPlotWindow:
    ...
    def _create_day_controls(self, parent):
        ...
        title_label = ttk.Label(
            parent, text="15-Day Energy Production Analysis", font=("Arial", 14, "bold")
        )
        title_label.grid(row=0, column=0, columnspan=4, pady=(0, 10))

        # Day navigation
        ttk.Label(parent, text="Select Day:").grid(
        ...
        self.day_info_label = ttk.Label(
            parent, text="Day 8 of 15 (Center Date)", font=("Arial", 10, "bold")
        )
        ...
        ttk.Button(
            button_frame, text="← Previous Day", command=self._previous_day
        ).grid(row=0, column=0, padx=(0, 5))

        ttk.Button(button_frame, text="Center Date", command=self._go_to_center).grid(
            row=0, column=1, padx=5
        )

        ttk.Button(button_frame, text="Next Day →", command=self._next_day).grid(
    ...
    def update_prediction_data(self, prediction_results: dict[str, Any]):
        ...
        try:
            ...
            logger.info(
                f"Updated plot data for installation: {prediction_results.get('installation_id')}"
            )
    ...
    def _plot_energy_production(
        self, hourly_data: pd.DataFrame, daily_summary: pd.DataFrame
    ):
        ...
        bars = self.energy_ax.bar(
            dates, daily_energy, alpha=0.7, color="orange", label="Daily Energy"
        )
        ...
        self.energy_ax.set_title(
            "Daily Energy Production (kWh)", fontsize=12, fontweight="bold"
        )
    ...
    def _plot_weather_data(self, hourly_data: pd.DataFrame):
        ...
        if "temperature_2m" in hourly_data.columns:
            daily_temps = hourly_data.groupby(hourly_data.index.date)[
                "temperature_2m"
            ].agg(["mean", "min", "max"])
        ...
        if "shortwave_radiation" in hourly_data.columns:
            daily_radiation = hourly_data.groupby(hourly_data.index.date)[
                "shortwave_radiation"
            ].sum()

            dates = daily_radiation.index
            self.radiation_ax.bar(
                dates,
                daily_radiation,
                alpha=0.7,
                color="gold",
            ...
            self.radiation_ax.set_title(
                "Solar Radiation (Wh/m²)", fontsize=10, fontweight="bold"
            )
    ...
    def _plot_rankings(self, daily_summary: pd.DataFrame):
        ...
        self.ranking_ax.set_title(
            "Energy Production Rankings", fontsize=10, fontweight="bold"
        )
    ...
    def _update_day_summary(self):
        ...
        try:
            ...
            if len(daily_summary) > self.current_day_index:
                ...
                self.day_labels["day_energy_label"].config(
                    text=f"{day_data['predicted_total_energy']:.1f} kWh"
                )
                self.day_labels["day_specific_label"].config(
                    text=f"{day_data['predicted_specific_energy']:.2f} kWh/kWp"
                )
                ...
                if len(day_hourly) > 0:
                    ...
                    self.day_labels["day_peak_label"].config(
                        text=f"{peak_hour.strftime('%H:%M')} ({peak_value:.2f} kWh/kWp)"
                    )

                # Weather info
                self.day_labels["day_temp_label"].config(
                    text=f"{day_data.get('temperature_2m', 0):.1f}°C"
                )
                self.day_labels["day_cloud_label"].config(
                    text=f"{day_data.get('cloud_cover', 0):.1f}%"
                )
                self.day_labels["day_radiation_label"].config(
                    text=f"{day_data.get('shortwave_radiation', 0):.0f} Wh/m²"
                )
                ...
                self.day_labels["day_ranking_label"].config(
                    text=f"{ranking}/5 - {ranking_text}"
                )
    ...
    def _update_period_statistics(self):
        ...
        try:
            ...
            self.period_labels["period_range_label"].config(
            ...
            self.period_labels["period_total_label"].config(
                text=f"{total_energy:.1f} kWh"
            )
            self.period_labels["period_avg_label"].config(
                text=f"{avg_daily:.1f} kWh/day"
            )
            ...
            self.period_labels["period_best_label"].config(
                text=f"{best_day} ({best_energy:.1f} kWh)"
            )
            self.period_labels["period_worst_label"].config(
                text=f"{worst_day} ({worst_energy:.1f} kWh)"
            )
    ...
    def _update_source_information(self):
        ...
        try:
            ...
            self.source_labels["source_model_label"].config(
            ...
            if model_name in performance:
                ...
                self.source_labels["source_mae_label"].config(
                    text=f"{mae_score:.3f} kWh/kWp"
                )
