...
class ComprehensiveDataProcessor:
        ...
        try:
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    logger.info(f"Loaded installation: {installation.installation_id} "
            ...
            for installation_id, installation in self.installations.items():
                ...
                if sheet_name in sheet_names:
                    try:
                        ...
                        numeric_columns = ['Produced Energy (kWh)', 'Specific Energy (kWh/kWp)', 'CO2 Avoided (tons)']
                    ...
                    logger.warning(f"No data sheet found for installation {installation_id} (sheet: {sheet_name})")
            
            logger.info(f"Successfully loaded energy data for {len(self.energy_data)} installations")
    ...
    def _load_weather_data(self):
        ...
        try:
            ...
            locations = set(installation.location for installation in self.installations.values())
        ...
        try:
            for installation_id, energy_df in self.energy_data.items():
                ...
                if location in self.weather_data:
                    ...
                    logger.info(f"Combined {len(combined)} records for {installation_id}")
    ...
    def _add_computed_features(self, df: pd.DataFrame, installation: InstallationInfo) -> pd.DataFrame:
        ...
        try:
            ...
            if 'temperature_2m' in df.columns and 'cloud_cover' in df.columns:
                df['temp_cloud_interaction'] = df['temperature_2m'] * (100 - df['cloud_cover']) / 100
                
            if 'shortwave_radiation' in df.columns and 'cloud_cover' in df.columns:
                df['radiation_cloud_interaction'] = df['shortwave_radiation'] * (100 - df['cloud_cover']) / 100
                
            # Power efficiency (actual vs theoretical)
            if 'Produced Energy (kWh)' in df.columns and 'shortwave_radiation' in df.columns:
                ...
                df['power_efficiency'] = df['power_efficiency'].clip(0, 1)  # Cap at 100% efficiency
    ...
    def get_installation_by_id(self, installation_id: str) -> Optional[InstallationInfo]:
    ...
    def get_locations(self) -> List[str]:
        ...
        return list(set(installation.location for installation in self.installations.values()))
        
    def get_installations_by_location(self, location: str) -> List[Tuple[str, InstallationInfo]]:
        """Get all installations for a specific location."""
        return [(id, info) for id, info in self.installations.items() if info.location == location]
...
if __name__ == "__main__":
    ...
    for inst_id, inst_info in processor.get_installation_list():
        print(f"  {inst_id}: {inst_info.installed_power_kwp} kWp in {inst_info.location}")