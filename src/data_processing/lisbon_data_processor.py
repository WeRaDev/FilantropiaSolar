...
class LisbonDataProcessor:
            ...
            for installation, sheet_names in self.correspondence.items():
                for sheet_name in sheet_names:
                    try:
                        ...
                        if 'Specific Energy (kWh/kWp)' not in df.columns and 'Produced Energy (kWh)' in df.columns:
                            # Assuming default kWp rating if not available
                            df['Specific Energy (kWh/kWp)'] = df['Produced Energy (kWh)'] / 10.0  # Default 10kWp
                        
                        # Add ranking
                        if 'Specific Energy (kWh/kWp)' in df.columns:
                            df['Ranking'] = calculate_specific_energy_ranking(df['Specific Energy (kWh/kWp)'])
                        ...
                        logger.error(f"Error loading {installation} from sheet {sheet_name}: {e}")
    ...
    def load_weather_data(self, weather_file="weather_files/Lisbon_weather.csv"):
        ...
        try:
            ...
            self.weather_data = pd.read_csv(file_path, delimiter=',', on_bad_lines='skip')
    ...
    def calculate_seasonal_correlations(self, installation):
        ...
        for season in ['Winter', 'Spring', 'Summer', 'Autumn']:
            ...
            if len(season_data) > 10:  # Minimum samples for correlation
                ...
                correlation_data = season_data[correlation_features].select_dtypes(include='number')
    ...
    def calculate_hourly_patterns(self, installation):
        ...
        hourly_patterns = merged_df.groupby('Hour').agg({
            'Produced Energy (kWh)': ['mean', 'std', 'count'],
            'Specific Energy (kWh/kWp)': ['mean', 'std'] if 'Specific Energy (kWh/kWp)' in merged_df.columns else [],
            'Ranking': ['mean', 'std'] if 'Ranking' in merged_df.columns else [],
            'temperature_2m (°C)': 'mean' if 'temperature_2m (°C)' in merged_df.columns else [],
            'shortwave_radiation (W/m²)': 'mean' if 'shortwave_radiation (W/m²)' in merged_df.columns else [],
            'cloud_cover (%)': 'mean' if 'cloud_cover (%)' in merged_df.columns else []
        }).round(2)
        
        # Flatten column names
        hourly_patterns.columns = ['_'.join(col).strip('_') for col in hourly_patterns.columns]
    ...
    def get_installation_summary(self, installation):
        ...
        summary = {
            ...
            'date_range': {
                'start': df['Date'].min().strftime('%Y-%m-%d') if df['Date'].notna().any() else None,
                'end': df['Date'].max().strftime('%Y-%m-%d') if df['Date'].notna().any() else None
            },
            'energy_production': {
                'total_kwh': df['Produced Energy (kWh)'].sum() if 'Produced Energy (kWh)' in df.columns else 0,
                'avg_hourly_kwh': df['Produced Energy (kWh)'].mean() if 'Produced Energy (kWh)' in df.columns else 0,
                'max_hourly_kwh': df['Produced Energy (kWh)'].max() if 'Produced Energy (kWh)' in df.columns else 0
            }
        }
    ...
    def get_all_installations_summary(self):
        ...
        if self.pv_data:
            ...
            summaries['combined'] = {
                'total_installations': len([k for k in self.pv_data.keys() if k in self.lisbon_installations]),
                'total_records': total_records,
                'total_energy_kwh': total_energy,
                'avg_energy_per_installation': total_energy / len(self.pv_data) if self.pv_data else 0
            }
    ...
    def prepare_prediction_data(self, installation, target_date, days_window=30):
        ...
        month_window = [(m - 1) % 12 + 1 if m <= 0 else (m - 1) % 12 + 1 if m > 12 else m for m in month_window]
        
        historical_data = merged_df[merged_df['Month'].isin(month_window)].copy()
        
        # Add features for prediction
        if not historical_data.empty:
            historical_data['DaysSinceEpoch'] = (historical_data['Date'] - pd.Timestamp('1970-01-01')).dt.days
            historical_data['SinHour'] = np.sin(2 * np.pi * historical_data['Hour'] / 24)
            historical_data['CosHour'] = np.cos(2 * np.pi * historical_data['Hour'] / 24)
            historical_data['SinMonth'] = np.sin(2 * np.pi * historical_data['Month'] / 12)
            historical_data['CosMonth'] = np.cos(2 * np.pi * historical_data['Month'] / 12)
