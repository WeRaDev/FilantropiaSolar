...
class InsufficientDataError(FilantropiaSolarError):
    ...
    def __init__(self, message: str, required_samples: int | None = None, available_samples: int | None = None, **kwargs):
        super().__init__(message, **kwargs)
...
def retry_sync(config: RetryConfig | None = None) -> Callable[[Callable[P, R]], Callable[P, R]]:
    """
...
def retry_async(config: RetryConfig | None = None) -> Callable[[Callable[P, R]], Callable[P, R]]:
    """
...
retry_weather_api = retry_async(RetryConfig(
    max_attempts=3,
    base_delay=2.0,
    retry_on_exceptions=(WeatherAPIError, ConnectionError, TimeoutError)
))

retry_data_processing = retry_sync(RetryConfig(
    max_attempts=2,
    base_delay=1.0,
