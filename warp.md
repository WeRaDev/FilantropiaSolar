...
- Nextcloud app is at v3.1.1 (`nextcloud-app/appinfo/info.xml`) and is the MAIN application (platform: Nextcloud + ML service + Odoo public site). **Repo reorganization (2026-07-29)**: desktop era moved to `desktop/` as the client edition — now at **v1.3.0 (API-client mode)**: consumes the Nextcloud API (`src/nextcloud_backend.py`, FS_SERVER_URL/USER/PASSWORD env), no local ML at runtime; local-ML modules retained as legacy. Desktop checks now run as `cd desktop && ruff format --check . && ruff check . && mypy && pytest`.
- Datasets: `data/` (lowercase, both Mendeley Excel files) and `weather_files/` are TRACKED at root — clean clones are self-contained; desktop reaches them via committed symlinks. `archive/` is a permanently tracked preservation area (stale files archived, never deleted). Rollback anchor: tag `pre-desktop-split-20260729`.
- Regression coverage: `tests/unit/test_regressions_w27.py` guards the four latent bugs fixed in the paydown.
- **TRL4/deploy readiness verified**: `connect-trl4.sh` joins `filantropia-nextcloud` + `filantropia-ml` to `compose_city_internal` (SolarSeed-v3 ops network); cross-network probes confirmed from a clean container (NC `status.php` healthy, ML `/health` with 9 models loaded). v3.1.1 deployed via `occ upgrade` (`needsDbUpgrade: false`); public API re-verified post-migration.
- Next tracks: (3) remaining TRL4 hardening per below, (4) desktop v1.2.4 based on nextcloud/desktop (explicitly queued last by user).
- The dated status sections below are retained as a running history log, not the current state.
...
### TRL4 connection notes (2026-07-28)
- `connect-trl4.sh` connections are Docker-network state: they do NOT survive container recreation. Re-run it after any `docker compose up -d --force-recreate` or full stack restart of the FilantropiaSolar side. Consider appending it to `setup.sh` for TRL4 machines.
- Probe addresses from the SolarSeed-v3 network: `http://filantropia-nextcloud:80` (Nextcloud) and `http://filantropia-ml:8501` (ML). Spirit/Prometheus in the SolarSeed-v3 stack scrape these; verify with `docker run --rm --network compose_city_internal alpine:3.20 sh -c "wget -qO- http://filantropia-ml:8501/health"`.
- After bumping the app version and recreating the stack, run `docker exec -u 33 filantropia-nextcloud php occ upgrade` — `status.php` shows `needsDbUpgrade: true` until migrations apply (public API returns errors meanwhile).

### Secrets handling (FS_PUBLIC_API_TOKEN)
- Source of truth: Nextcloud app config `filantropia_solar/public_api_token` (generated by `setup.sh` if absent, 48 hex chars).
- Export: `setup.sh` writes it to `SolarSeed-v3/.secrets/filantropia_public_api_token` with mode 600 — never committed, never echoed.
- Consumers: the Odoo container reads it via `FS_PUBLIC_API_TOKEN` env (`docker compose --profile odoo up -d` with the var exported); public API clients send `Authorization: Bearer <token>`.
- Rotation: set a new value via `occ config:app:set filantropia_solar public_api_token --value=<new>` and re-run `setup.sh` to re-export; recreate the Odoo container to pick it up.
